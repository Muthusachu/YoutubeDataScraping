...
#Creating function for collecting Playlist details
def Getting_playlist_details(channel_id):
  Play_lists=[]
  Next_page=None
  while True:
    response5=youtube.playlists().list(
    ...
    for i in response5["items"]:
      data=dict(playlist_id=i["id"],
                ...
                Publist_at=i["snippet"]["publishedAt"],
                Video_count=i["contentDetails"]["itemCount"])
      Play_lists.append(data)
    Next_page=response5.get("nextPageToken")
...
uri = "mongodb+srv://muthu03:03091996@cluster0.fyjmz7m.mongodb.net/?retryWrites=true&w=majority"
...
client=MongoClient("mongodb+srv://muthu03:03091996@cluster0.fyjmz7m.mongodb.net/?retryWrites=true&w=majority")
...
def channel_information(channel_id):
    ...
    pl_dt=Getting_playlist_details(channel_id)
    vd_dt=Getting_video_info(pid_dt)
    cm_dt=Getting_comment_info(pid_dt)
    
    collect.insert_one({"Channel_information":ch_dt,"Playlist_information":pl_dt,
                        "Video_information":vd_dt,"Comment_information":cm_dt})
...
#creating table for playlist details in sql database
...
def Playlist_Table():
    ...
    try:
        mycursor.execute('''create table Playlists(playlist_id varchar(100) primary key,
                                            Title varchar(100) ,
                                            Channel_id varchar(100),
                                            Channel_name varchar(100),
                                            Publist_at timestamp,
                                            Video_count int)''')
        mydb.commit()
        ...
        # creat dataframe in pandas
    pl_list=[]
    dt2=collect.find({},{"_id":False,"Playlist_information":True})
    for pl_data in dt2:
    ...
    for index,row in df2.iterrows():
        ...
        Insert_val_in_col=('''insert into Playlists(playlist_id,
                                        Title,
                                        Channel_id,
                                        Channel_name,
                                        Publist_at,
...
def comment_Table():
    ...
    mycursor.execute('''create table if not exists comment(Comment_Id varchar(200) primary key,
...
#creating function to display channel information fron mongodm data base and display in streamlit
...
#creating function to display playlist information fron mongodm data base and display in streamlit
...
if st.button("Store data in a MongoDB"):
    ...
    if Channel_Id not in ch_ids:              
        ...
        st.success("Channel details is already exist in MongoDb")
...
Show_table=st.radio("Select Table to view",("Channels","playlist","Videos","Comments"))
